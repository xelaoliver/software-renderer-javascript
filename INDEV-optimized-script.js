...
function vert(x, y, z) {
  vertdistance.push(
    Math.sqrt(
      Math.pow(x - camera.x, 2) +
        Math.pow(y - camera.y, 2) +
        Math.pow(z - camera.z, 2)
    ) +
      Math.random() / 100
  );
  ...
  x =
    Math.sin((camera.rotx * Math.PI) / 180) * store.z +
    Math.cos((camera.rotx * Math.PI) / 180) * store.x;
  store.z =
    Math.cos((camera.rotx * Math.PI) / 180) * store.z -
    Math.sin((camera.rotx * Math.PI) / 180) * store.x;

  y =
    Math.sin((camera.roty * Math.PI) / 180) * store.z +
    Math.cos((camera.roty * Math.PI) / 180) * store.y;
  z =
    Math.cos((camera.roty * Math.PI) / 180) * store.z -
    Math.sin((camera.roty * Math.PI) / 180) * store.y;
  ...
  if (type === 'z') {
    for (let i = 0; i < polygon.length; i += 3) {
      let coords = {
        ...
        y1: polygon[i + 1],
        z1: polygon[i + 2],
        x2: polygon[(i + 3) % polgyon.length],
        y2: polygon[(i + 4) % polgyon.length],
        z2: polygon[(i + 5) % polgyon.length],
      };
